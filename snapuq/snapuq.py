...
class SnapUQ(nn.Module):
    ...
    def remove_hooks(self) -> None:
        """Remove all registered forward hooks (call before deleting or re-registering)."""
    ...
    def gaussian_nll_loss(self, x: torch.Tensor) -> torch.Tensor:
        ...
        The backbone forward is run under torch.no_grad() since its parameters
        are frozen during post-hoc SNAP-UQ training.
        """
        # Run backbone (and hooks) with no gradient through backbone weights
        with torch.no_grad():
            _ = self.backbone(x)
        # Activations are now cached in self._acts; build pooled representations
        if len(self.predictors) == 0 or self._tap_weights is None:
            raise RuntimeError("Call infer_dims(example_input) once before using gaussian_nll_loss.")
        pooled = [global_avg_pool(self._acts[i]) for i in range(len(self.tap_modules))]
        tap_nll_vals = []
        for i, pred in enumerate(self.predictors):
            a_prev = pooled[i].detach()
            a_next = pooled[i + 1].detach()
            mu, var = pred(a_prev)
            nll = 0.5 * (((a_next - mu) ** 2) / (var + 1e-12) + torch.log(var + 1e-12)).sum(dim=-1) / max(1.0, float(a_next.shape[-1]))
            tap_nll_vals.append(nll)
        loss = torch.stack([t.mean() for t in tap_nll_vals]).mean()
        return loss
