...
def main():
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            snap_out = suq(x, return_uq=True)
            logits = snap_out.logits.detach().cpu().numpy()[0]
            y0 = int(y.numpy()[0])
            pred = int(np.argmax(logits))
            correct.append(1 if pred == y0 else 0)
            score = snap_out.U.detach().cpu().numpy()[0] if snap_out.U is not None else snap_out.S.detach().cpu().numpy()[0]
            if np.ndim(score) > 0:
