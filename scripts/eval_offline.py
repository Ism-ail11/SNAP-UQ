...
def main():
    ...
    # If U exists, use it; else fall back to S as uncertainty score.
    # U is always 1-D after collect_scores; guard against accidental 2-D arrays.
    _U = id_scores["U"]
    if _U is not None:
        score = _U[:, 0] if _U.ndim > 1 else _U
    else:
        score = id_scores["S"]
    ...
    if ood_cfg:
        ...
        if ood_ds_name:
            ...
            _oU = ood_scores["U"]
            if _oU is not None:
                ood_score = _oU[:, 0] if _oU.ndim > 1 else _oU
            else:
                ood_score = ood_scores["S"]
